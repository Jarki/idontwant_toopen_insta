...
commands also require both runtime role names so Alembic can preserve their
isolation from migration metadata:
...
4. Run the migration against a disposable database:
   ...
   ```bash
   DB_APP_USER=db_app DB_ERROR_API_USER=db_error_api \
   DATABASE_URL=postgresql+psycopg://db_migration:pass@localhost:5432/test_db \
     uv run poe db-upgrade
   ```
