...
class MediaFetchService:
    ...
    def fetch(self, match: ResolvedUrlMatch) -> MediaFetchResult:
        ...
        if cached is not None and _is_reusable(cached):
            return MediaFetchResult(media=cached, url=match.url)

        download_result = match.downloader.download(
            match.url,
        ...
        if not _identity_matches(download_result.media, ref):
            ...
            return MediaFetchResult(media=None, url=match.url, failure_reason="unknown")

        self.repository.insert_media(download_result.media)
        return MediaFetchResult(media=download_result.media, url=match.url)
