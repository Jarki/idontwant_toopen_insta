...
`downloaders/x.py` supports X and legacy Twitter status URLs. Video format selection chooses the best format whose exact or estimated size is at most 100 MB, and downloads proceed only when the selected-format metadata reports that size; runtime progress and final-file checks provide defense in depth. Photo and text-only posts fall back to X page metadata; photo downloads use an exact X CDN allowlist and a 20 MiB streaming limit. Text-only posts are cached without assets and sent as Telegram text messages.
