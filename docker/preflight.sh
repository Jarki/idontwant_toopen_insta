...
check_var "DB_ERROR_API_USER"    "Error API database user"
check_var "DB_ERROR_API_PASSWORD" "Error API database password"
...
for ((i = 0; i < ${#roles[@]}; i++)); do
    for ((j = i + 1; j < ${#roles[@]}; j++)); do
        if [ -n "${roles[i]}" ] && [ "${roles[i]}" = "${roles[j]}" ]; then
            echo "::error::FATAL: PostgreSQL bootstrap, migration, application, and Error API roles must be distinct"
            errors=$((errors + 1))
        fi
    done
done
